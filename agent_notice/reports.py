...
def render_daily(day: date, items: Sequence[Item], failures: Sequence[str] = ()) -> str:
    lines = [f"# Agent 技术日报 · {day.isoformat()}", ""]
    for category in ORDER:
        lines.extend((f"## {category.value}", ""))
        section = [f"- [{item.title}]({item.url}) — {item.summary}" for item in items if item.category is category]
